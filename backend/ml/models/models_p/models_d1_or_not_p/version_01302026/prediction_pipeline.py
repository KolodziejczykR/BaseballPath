#!/usr/bin/env python3
"""
Pitcher D1 Prediction Pipeline - Production Version
...
def _infer_missing_flags(df: pd.DataFrame) -> pd.DataFrame:
    """Ensure *_missing flags exist based on pitch features."""
...
def _infer_num_pitches_and_diffs(df: pd.DataFrame) -> pd.DataFrame:
    """Ensure num_pitches and FB-secondary diffs exist."""
    velo_cols = [
        "FastballVelo Range",
        "Changeup Velo Range",
        "Curveball Velo Range",
...
def _prepare_features(player_data: Dict, feature_list: List[str]) -> pd.DataFrame:
    """Build a single-row DataFrame with required features."""
    # Accept both Region and player_region keys
...
def predict_pitcher_d1_probability(player_data: dict, models_dir: str) -> D1PredictionResult:
    """
    Predict D1 probability for pitchers.
    """
    ...
    d1_probability = float(model.predict_proba(X)[:, 1][0])
    d1_prediction = d1_probability >= threshold

    confidence = "High" if d1_probability >= 0.7 or d1_probability <= 0.3 else "Medium"
