...
def _create_player_from_info(player_info: Dict[str, Any]):
    ...
    if position in ["RHP", "LHP", "P", "PITCHER"]:
        ...
        return PlayerPitcher(
            height=player_info.get("height", 72),
            weight=player_info.get("weight", 180),
            primary_position="LHP" if throwing_hand == "L" else "RHP",
            throwing_hand=throwing_hand,
            region=player_info.get("region", "West"),
            fastball_velo_range=player_info.get("fastball_velo_range"),
            fastball_velo_max=player_info.get("fastball_velo_max"),
            fastball_spin=player_info.get("fastball_spin"),
            changeup_velo=player_info.get("changeup_velo"),
            changeup_spin=player_info.get("changeup_spin"),
            curveball_velo=player_info.get("curveball_velo"),
            curveball_spin=player_info.get("curveball_spin"),
