"""Validate PCI tier placement for real Non-P4 D1 outfielder commits."""
...
def test_non_p4_d1_outfielders_tier_accuracy(hitter_df):
    players = sample_hitters(hitter_df, tier=NON_P4_D1, pos_group="OF", n=100)
    results = [run_pci(p, is_pitcher=False) for p in players]
    assert_tier_accuracy(
        results,
        min_correct_pct=70.0,
        allow_adjacent=True,
        label="Non-P4 D1 Outfielders",
    )
