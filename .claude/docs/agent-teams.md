...
When the root cause is unclear, making teammates explicitly adversarial ensures multiple theories are tested:
...
> **💡 Tip:** The lead breaks work into tasks and assigns them to teammates automatically. If it isn't creating enough tasks, ask it to split the work into smaller pieces.
...
Check their output using Shift+Down in in-process mode or by clicking the pane in split mode, then either:
...
* **Manual parallel sessions**: Git worktrees let you run multiple Claude Code sessions
